    ...
    <div className="relative overflow-hidden w-full h-[400px] bg-black flex items-center">
      <div className={`flex gap-6 animate-scroll ${paused ? "pause cursor-pointer" : ""}`} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
        {[...projects, ...projects].map((p, index) => (
          <div key={index} className="w-52 sm:w-60 md:w-65 lg:w-65 h-40 sm:h-44 md:h-45 lg:h-45 bg-gray-500/20 border border-green-600 rounded-md p-4 flex flex-col justify-between text-white hover:scale-110 hover:-translate-y-2 transition-transform duration-300">
            <h3 className="text-md font-semibold">{p.title}</h3>
            <div className="text-[#FEB57F] font-bold">{p.techStack}</div>
            <div className="flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-user-icon lucide-user my-2 text-[#FF8162]"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
